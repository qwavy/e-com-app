...
import { CustomClientResponse, UpdatePersonalInfoCustomerProps } from '@shared/types/customerTypes';

export const updatePersonalInfoAction = async ({ data, id, version }: UpdatePersonalInfoCustomerProps) => {
  try {
    const response = (await updatePersonalInfoCustomer({ data, id, version })) as CustomClientResponse;
